...
| R6 | **Acceptance-artifact prerequisites for FULL-assurance GATE** (EXECUTION-PROTOCOL *Assurance levels*) | today's S3 froze **one witness per REQ**, pure-type `ref/*.ts` oracles, and no `properties-*.md` → GATE runs at **FLOOR** assurance (mutation + witness + diff-scope + purity). FULL assurance requires S3/scaffold to add (a) ≥2 independent fixtures per behavioural REQ, (b) an executable reference oracle **or** a frozen property set. Until then the seal records `assurance: FLOOR` honestly — never an inert leg reported as passed. | S3 + scaffold (a declared method-prerequisite, off critical path) |
