...
goal-trace: "there is exactly one way in and back-channel writes cannot surface → the governance surface is four tools with atlas-emit the only write path over a permissioned store → the write-door interface"
vertical: TOOLS (governance surface is exactly four tools · atlas-emit the only write path · reject back-channel writes · read projections carry no write authority · tools pure and total, malformed fails closed · store medium append-only/permissioned · reads reject ungrounded rows · direct write never surfaces as a served fact) — demoable: a direct store write bypassing atlas-emit is rejected and never surfaces; a read projection cannot write
reqs: [ REQ-TOOLS-1a, REQ-TOOLS-1b, REQ-TOOLS-1c, REQ-TOOLS-1d, REQ-TOOLS-2a, REQ-TOOLS-2b, REQ-TOOLS-15a, REQ-TOOLS-15b, REQ-TOOLS-15c ]
campaign: CAMPAIGN-7
