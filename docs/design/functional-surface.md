...
**Extensions:** 2a. subtree changed → **DRIFTED**; predicate → CHK re-runs check: HOLDS→re-ground, else **BROKEN**. 2b. advisory (no check) → cannot mechanically decide → serve **STALE**/flagged, non-blocking (never auto-reground silently, never block, `GROUND-13`). **2c. bytes changed but *normalized* subtree identical (reformat/import/comment/unrelated-rename) → stays FRESH, no re-check, no flag (`spec §3.1`, acceptance #1) — the precision side that prevents drift-thrash.** 3a. forward-dependency's signature/type changed → F's rState drifts transitively (`GROUND-11`). 4a. F BROKEN and cited by the edit → block the merge (`A-3`).
...
**Extensions:** 2a. unstable LLM (anchor,slot) → primaryAnchor computed **mechanically** from referenced symbols + near-dup probe → MERGE-or-justify-CREATE (anti-landfill). 3a. T0 candidate → human-only ratify. 3b. un-grounded / true-but-obvious → rejected at the 2-door (truth AND usefulness). 3c. **delta conflicts with a concurrent seat's delta → mechanical merge on contentHash, else surface as a knowledge-conflict for review** (see UC-G33). 4a. transcript has a secret → cred-scrub redacts at source, **blocks** the write (`PERSIST-10a`).
...
**MSS:** 1) jimmy proposes candidate + grounding receipt. 2) 2-door check (grounded? useful-non-obvious?). 3) tier routes: T1/T2 → lead + lucy/bobby veto; T0 → human-only. 4) admit, or ABSTAIN (why-not grounded, first-class).
