...
- **D-TV4: the `shape` vocabulary is closed/additive-only.** `assertion-only-in-catch` and `no-assertion-in-test` today; a new shape is a `cv` bump, exactly like `RelationKind`. The normative list lives in `reference/atlas-knowledge.md` — a member added to the union without a row there is an undocumented widening.
