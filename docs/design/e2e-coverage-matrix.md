...
- **rev-index EMPTY-on-contention (open, follow-up — task #73)** — `createRevIndex` fails closed to `EMPTY_AXES` on a transient `git worktree add` failure under concurrent load → resolvers return undefined → drift silently dropped → reconcile exit 0 (green) despite real drift (flaky ~1/20). Pre-existing; a transient failure should be distinguishable from a genuine empty (retry/surface, not silently under-report + pass the gate). bobby (N10 review).
