...
- **Ground:** each served fact carries a StructRef anchored to a BLAKE3 **subtree** hash; an edit that changes
  the cited unit's **normalized subtree** flips it stale → withheld/flagged (a reformat / added-import /
  unrelated rename inside the unit stays FRESH — drift keys on the subtree, not "any edit in range"; spec §3.1,
  acceptance #1). This is why the retrieval is **auditable** where an embedding is not.
