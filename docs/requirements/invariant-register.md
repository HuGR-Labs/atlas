...
| KNOW-8 propose ≠ ratify | FR-9 | COUPLED-with FR-7 (DP-11) | ref-model | 0 explorer writes reach store except via ratifier — **HOLDS, and holds VACUOUSLY as of task #83's measurement: 0 explorer writes reach the store AT ALL, by any route.** Nothing reads staging back (`loadStaging` had zero production callers and is deleted; there is no `promote` command), so what enforces this today is SEVERANCE, not ratification — the ratifier is not the gate on that path, the absence of a path is. The explorer's durable write to its OWN sidecar (`commitStaging`) is built and real; the governed PROMOTION door out of it is not. A-D4 | autonomy↔gated quality | behavioural | ADR-KN8 | **RATIFIED — measurable holds vacuously; see A-D4** |
