...
| **KNOW-8** Propose ≠ ratify | Miner ≠ acceptor. | The explorer MAY write only **candidates** (staging); ratification is the reconcile/lead's, with reviewer veto and billy required for `T0`. A **grounded, low-risk, `T2` advisory** candidate MAY be ratified by the deterministic **fast-path** (KNOW-18) with no human; `T0` / contested / **all predicate** candidates MUST get full human ratification. <br>⚠️ **AS BUILT TODAY (measured, task #83 — A-D4):** the first clause is real — `atlas mine` writes candidates durably to its own sidecar through `commitStaging`, and never touches the projection. The RATIFICATION clause is **not wired**: nothing reads staging back (no `promote` command; the staging read leg does not exist), so a staged candidate has no route into governed knowledge at all. The invariant's measurable ("0 explorer writes reach the store except via a ratifier") therefore holds **vacuously** — enforced by SEVERANCE, not by ratification. This describes the current state, not a change of design: the governed promotion door is tracked as its own task. | A-7 |
...
| **KNOW-11** Owner + scope | Read universal, write owned. | Every fact MUST carry an `owner` + `scope`; read is universal (any agent/human), write is the owner's (the class member for that territory). | §ownership |
