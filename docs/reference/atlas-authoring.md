...
| **A-D2** | MCP advertises `GOVERNANCE_SURFACE ∪ READ_SURFACE`; the already-invocable read doors (`doctor`, `node`) join it | **proposed** — ADR-0005 (reconciled 2026-08-24); owner-directed. `READ_SURFACE` is SIX members — `anchors`/`slots`/`draft`/`check`/`doctor`/`node`. `diff` does NOT join: it is a declared zero-caller reference model (`packages/tools/src/diff.ts`), unwired to any transport — ARCH-5 (advertised≡invocable) forbids advertising it until it is genuinely wired, in a later WP |
