...
// ── the tier LATTICE — RE-EXPORTED from @atlas/contracts (task #84) ────────────────────────────────────
//
...
//
// The lattice itself lives at L0 BESIDE the type it orders (`@atlas/contracts/tier.ts`), not here, because
// the read side needs it too and `retrieval` (L5) cannot import `knowledge` (L4)'s sibling without inverting
// the DAG. Housing the order away from the type is what let four modules rebuild it privately and each get
// `undefined` from an off-lattice value. Re-exported here so this facet stays the one place a ratification
// question is asked.
export { TIER_ORDER, isTier, tierRank, isWeakerTier, strictestTier } from '@atlas/contracts';
