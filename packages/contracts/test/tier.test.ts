// @atlas/contracts — test/tier.test.ts  (task #84 · the governance-class LATTICE, at the layer that owns it)
//
...
import { TIER_ORDER, isTier, tierRank, isWeakerTier, strictestTier } from '../src/tier.js';
import type { Tier } from '../src/tier.js';
...
describe('@atlas/contracts tier — the governance-class lattice (isTier · tierRank · isWeakerTier · strictestTier)', () => {
  it('SCN-TIER-1 — `isTier` admits EXACTLY the three real classes, byte-exact, prototype-proof', () => {
    ...
    for (const t of TIER_ORDER) expect(isTier(t), `${t} is a real class`).toBe(true);
    for (const bogus of OFF_LATTICE) {
      expect(isTier(bogus), `${String(bogus)} must not read as a governance class`).toBe(false);
    }
  });

  it('SCN-TIER-2 — `TIER_ORDER` is the total order STRICTEST-FIRST, and `tierRank` agrees with it', () => {
    // MUTANT: reverse TIER_ORDER (or drop T1) and the strictest-first contract every doc/sort reads breaks.
    expect([...TIER_ORDER]).toEqual(['T0', 'T1', 'T2']);
  ...
  it('SCN-TIER-3 — an off-lattice class ranks LAST, never `NaN` and never FIRST (a poisoned row sinks)', () => {
    // MUTANT: drop the `isTier` guard in `tierRank` and `indexOf` returns -1 — which sorts a poisoned row
    // AHEAD of a billy-ratified T0 node instead of behind every real class.
    for (const bogus of OFF_LATTICE) {
      expect(tierRank(bogus), `${String(bogus)} ranks last`).toBe(TIER_ORDER.length);
      expect(Number.isNaN(tierRank(bogus))).toBe(false);
      for (const t of TIER_ORDER) expect(tierRank(t)).toBeLessThan(tierRank(bogus));
    }
  });
  ...
  it('SCN-TIER-4 — an UNRECOGNIZED INCUMBENT is ALWAYS strictest: a node whose class cannot be read is never written', () => {
    ...
    for (const incumbent of OFF_LATTICE) {
      for (const declared of [...TIER_ORDER, ...OFF_LATTICE]) {
        expect(
  ...
  it('SCN-TIER-5 — an UNRECOGNIZED DECLARED class is ALWAYS weaker (nothing off-lattice writes anything)', () => {
    ...
    for (const declared of OFF_LATTICE) {
      for (const incumbent of TIER_ORDER) {
  ...
  it('SCN-TIER-6 — ON the lattice it is the strict order: weaker DOWN only, never reflexive, never up', () => {
    ...
    for (const t of TIER_ORDER) expect(isWeakerTier(t, t), `${t} re-stated is not weaker`).toBe(false);
