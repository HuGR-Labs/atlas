...
- Restating the signature, the types, or the name is not a fact. Neither is a summary of what the code
  plainly does.
- If this unit holds no such fact, output the single token `NO-FACT` and nothing else. Most units hold
  none. That is a correct, expected result — it is recorded as a deliberate abstention, never as a failure.
  Do NOT explain, apologise, or describe why: the bare token `NO-FACT` IS the abstention.
