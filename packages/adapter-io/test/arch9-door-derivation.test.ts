...
describe('§2 ARCH-9 for `tier` — the door supplies the class it DERIVED, not the one declared', () => {
  it('the ctx builder is real: a derived T0 sends a DECLARED-T2 advisory to full ratification', () => {
    ...
    const view = { ...declaredT2, slot: declaredT2.kind === 'relation' || declaredT2.kind === 'negation' ? undefined : declaredT2.predicateSlot } as unknown as Candidate;
    ...
    const t0View = { ...declaredT0, slot: declaredT0.kind === 'relation' || declaredT0.kind === 'negation' ? undefined : declaredT0.predicateSlot } as unknown as Candidate;
